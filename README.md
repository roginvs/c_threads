...
Так же текущая реализация использует неинвертированный порядок битов в подсчёте остатка. Инверсия происходит через отдельную таблицу для каждого входного байта. Так же инверсия выполняется и для накопителя. Это можно улучшить если отрефакторить создание таблицы и само деление на инверсию по-умолчанию. Это так же должно немного упростить код для деления.
